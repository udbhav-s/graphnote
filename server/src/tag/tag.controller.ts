import { Controller, UseGuards, UseInterceptors, Get, Param, Req, ForbiddenException, ParseIntPipe, Body, Post, Put, NotFoundException, Delete } from '@nestjs/common';
...
export class TagController {
  ...
  async create(
    ...
    if(!await this.workspaceService.canAccess(body.workspaceId, req.user.id)) {
      throw new ForbiddenException();
    }
  ...
  async update(
    ...
    let tag = await this.tagService.getById(id);
    if (!tag) throw new NotFoundException();
    if(!await this.workspaceService.canAccess(tag.workspaceId, req.user.id)) {
      throw new ForbiddenException();
    }
